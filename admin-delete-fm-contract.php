...
$stmt = $conn->prepare('SELECT pdf_path FROM fm_mobility_contracts WHERE id = ? LIMIT 1');
...
$stmt = $conn->prepare('DELETE FROM fm_mobility_signatures WHERE contract_id = ?');
$stmt->bind_param('i', $contractId);
$stmt->execute();
$stmt->close();

$stmt = $conn->prepare('DELETE FROM fm_mobility_contracts WHERE id = ? LIMIT 1');
$stmt->bind_param('i', $contractId);
$stmt->execute();
$stmt->close();

header('Location: admin-fm-contracts.php?deleted=1');
exit;
