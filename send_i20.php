<?php
/**
 * Send one email with multiple I-20 PDFs (one per selected university).
...
for ($i = 0; $i < $nInputs; $i++) {
    ...
    if ($uid < 1) {
        exit('Each I-20 needs a university selected.');
    }
    ...
    $origName = (string) ($files['name'][$i] ?? 'i20.pdf');
    ...
    if ($mime !== '' && stripos($mime, 'pdf') === false && $mime !== 'application/octet-stream') {
        exit('Only PDF files are allowed for I-20 documents.');
    }

    $uploadDir = __DIR__ . '/i20_documents/';
    if (!is_dir($uploadDir)) {
        mkdir($uploadDir, 0777, true);
    }
    $filename = uniqid('i20_', true) . '.pdf';
    $filepath = $uploadDir . $filename;

    if (!move_uploaded_file($tmp, $filepath)) {
        exit('Failed to save one of the uploaded I-20 files on the server.');
    }
...
if (count($documents) < 1) {
    exit('Add at least one university and attach its I-20 PDF.');
}
...
if (xander_env_get('SMTP_PASSWORD') === '') {
    ...
    error_log('I-20 email failed: SMTP_PASSWORD is not set in .env');
...
try {
    ...
    $subjectBase = $count > 1
        ? "Your Form I-20 documents ($count universities)"
        : ('Your Form I-20 — ' . $documents[0]['name']);
    $mail->Subject = '=?UTF-8?B?' . base64_encode($subjectBase) . '?=';

    $mail->Body = '
        <p>Dear ' . htmlspecialchars($studentName, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . ',</p>
        <p>Please find attached your <strong>Form I-20</strong> for the following institution(s):</p>
        <ul>' . $uniListHtml . '</ul>
        ' . $programHtml . '
        <p>Review the document carefully and keep it for your SEVIS / visa process.</p>
    ...
    // Mark applicant as I-20 Sent only after the email is sent successfully.
    $allFlags = [
    ...
    if (!in_array('i20_sent', $existing, true)) {
        foreach ($documents as $a) {
            @unlink($a['path']);
        }
        exit('Email sent, but i20_sent column is missing.');
    }
    $resetFlags = implode(', ', array_map(static fn ($f) => "`$f` = 0", $existing));
    $updateSQL = "UPDATE `$table` SET $resetFlags, `i20_sent` = 1 WHERE id = ?";
    $stmtUp = $conn->prepare($updateSQL);
    if (!$stmtUp) {
        foreach ($documents as $a) {
            @unlink($a['path']);
        }
        error_log('I-20 email sent but DB update prepare failed: ' . $conn->error);
    ...
    if (!$stmtUp->execute()) {
        ...
        error_log('I-20 email sent but DB update failed: ' . $conn->error);
        exit('Email sent, but failed to update applicant status. Please refresh and check the record.');
    }
    $stmtUp->close();
    ...
    $info = $e->getMessage();
    error_log('I-20 email failed: ' . $info);
    ...
    error_log('I-20 email failed: ' . $e->getMessage());
    exit('mail_error');
}
