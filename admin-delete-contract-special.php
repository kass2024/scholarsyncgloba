...
/* FETCH PDF PATH */
$stmt = $conn->prepare("SELECT pdf_path FROM student_contracts_special WHERE id = ? LIMIT 1");
...
/* DELETE DB RECORD */
$stmt = $conn->prepare("DELETE FROM student_contracts_special WHERE id = ? LIMIT 1");
$stmt->bind_param("i", $contractId);
$stmt->execute();
$stmt->close();

/* REDIRECT BACK */
header("Location: admin-contracts-special.php?deleted=1");
exit;
