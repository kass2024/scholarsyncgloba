...
/**
 * South Korea Event Participation — live file upload (passport or CV).
 */
...
try {
    ...
    if (!in_array($field, ['passport', 'cv'], true)) {
        $field = 'file';
    }
    ...
    $permDir = __DIR__ . '/uploads/korea_event';
    if (!is_dir($permDir) && !mkdir($permDir, 0755, true) && !is_dir($permDir)) {
        throw new RuntimeException('Cannot create upload directory');
    }

    $filename = 'kep_' . date('Ymd') . '_' . bin2hex(random_bytes(8)) . '_' . $field . '.' . $ext;
    ...
    echo json_encode([
        ...
        'file_path' => 'uploads/korea_event/' . $filename,
