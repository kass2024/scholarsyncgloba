...
$stmt = $conn->prepare('SELECT id, pdf_path, status FROM korea_invitation_contracts WHERE contract_token = ? LIMIT 1');
...
if ($pdfPath === '' || !is_file($pdfPath)) {
    ...
    try {
        ...
        if ($pdfPath !== '' && is_file($pdfPath)) {
            $update = $conn->prepare('UPDATE korea_invitation_contracts SET pdf_path = ? WHERE id = ?');
            if ($update) {
...
$filename = 'korea_invitation_contract_' . preg_replace('/[^a-zA-Z0-9_-]/', '_', $token) . '.pdf';
