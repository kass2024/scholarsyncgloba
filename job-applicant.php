...
// Clean Dashboard for ScholarSync Global Job Applicants
...
$jobProcessFilterActive = $processStatusFilter !== '' && xander_is_valid_job_process_status($processStatusFilter);
$jobFilterLabel = $jobProcessFilterActive ? ($JOB_PROCESS_STATUSES[$processStatusFilter] ?? $processStatusFilter) : '';
...
<head>
    ...
    <title>Job Applicants - ScholarSync Global</title>
    ...
    <div class="dashboard-container">
        ...
        <div class="dashboard-header">
            <div class="header-title">
                ...
                <p>Review and manage all job applications and documents<?php if ($jobProcessFilterActive): ?> — <strong style="color:#b91c1c;"><?= htmlspecialchars($jobFilterLabel) ?></strong><?php endif; ?></p>
                <?php if ($jobProcessFilterActive): ?>
                <p class="small" style="margin-top:8px;"><a href="job-applicant.php" style="color:#3661B9;font-weight:600;"><i class="fas fa-list"></i> View all applicants</a></p>
                <?php endif; ?>
            </div>
        ...
        <div class="search-container">
            ...
            <form method="GET" action="" class="search-form">
                ...
                <?php if(!empty($searchTerm)): ?>
                <a href="<?= $jobProcessFilterActive ? htmlspecialchars('?' . http_build_query(['process_status' => $processStatusFilter])) : '?' ?>" class="clear-btn">
            ...
            <?php if(empty($applicants)): ?>
                <div class="empty-state">
                    ...
                    <?php if(!empty($searchTerm)): ?>
                        ...
                        <a href="<?= $jobProcessFilterActive ? htmlspecialchars('?' . http_build_query(['process_status' => $processStatusFilter])) : '?' ?>" class="search-btn" style="display: inline-flex;">
                            <i class="fas fa-redo"></i> Show All Applicants
                        </a>
                    <?php elseif ($jobProcessFilterActive): ?>
                        <p>No applicants currently have status &ldquo;<?= htmlspecialchars($jobFilterLabel) ?>&rdquo;.</p>
                        <a href="job-applicant.php" class="search-btn" style="display: inline-flex;">
                            <i class="fas fa-redo"></i> View all applicants
                        </a>
                    <?php else: ?>
