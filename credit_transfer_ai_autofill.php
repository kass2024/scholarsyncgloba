...
function map_student_document_to_credit_attachment(string $studentType): string
{
    return match ($studentType) {
        'valid_passport', 'birth_certificate' => 'passport_or_id',
        'degree_transcripts', 'english_certificate' => 'current_transcripts',
        'high_school_degree' => 'current_degree',
        'cv_resume', 'recommendation_letters', 'personal_statement' => 'academic_cv',
        'payment_proof' => 'payment_proof',
        default => '',
    };
}

function credit_transfer_attachment_labels(string $lang): array
{
    return [
        'current_degree' => $lang === 'fr' ? 'Certificat de diplôme actuel' : 'Current Degree Certificate',
        'current_transcripts' => $lang === 'fr' ? 'Relevés de notes actuels' : 'Current Academic Transcripts',
        'passport_or_id' => $lang === 'fr' ? 'Passeport ou pièce d\'identité' : 'Passport or National ID',
        'academic_cv' => $lang === 'fr' ? 'CV académique' : 'Academic CV / Résumé',
        'payment_proof' => $lang === 'fr' ? 'Preuve de paiement' : 'Payment Proof',
    ];
}

function map_merged_student_fields_to_credit_form(array $merged): array
{
    $out = [];
    foreach (['first_name', 'middle_name', 'last_name', 'email'] as $k) {
        if (!empty($merged[$k])) {
    ...
    if (!empty($merged['dob'])) {
        $ts = strtotime((string)$merged['dob']);
        if ($ts !== false) {
            $out['birth_year'] = date('Y', $ts);
            $out['birth_month'] = date('m', $ts);
            $out['birth_day'] = date('d', $ts);
        }
    }
    if (!empty($merged['address_line1'])) {
        $out['street_address'] = (string)$merged['address_line1'];
    }
    if (!empty($merged['address_line2'])) {
        $out['address_line_2'] = (string)$merged['address_line2'];
    }
    ...
    if (!empty($merged['area_code']) && !empty($merged['phone_number'])) {
        $mobile = trim((string)$merged['area_code'] . ' ' . (string)$merged['phone_number']);
    } elseif (!empty($merged['phone_number'])) {
        $mobile = (string)$merged['phone_number'];
    }
    if ($mobile !== '') {
        $out['mobile_number'] = $mobile;
    }
...
$creditSessionId = trim((string)($_SESSION['credit_user_id'] ?? ''));
if ($applicationId === '' || $creditSessionId === '' || !hash_equals($creditSessionId, $applicationId)) {
    $debug['api_key_status'] = 'not_checked';
    add_stage($debug, 'prepare', 'Credit transfer session mismatch.');
    json_exit([
        'status' => 'error',
...
$creditAttachmentLabels = credit_transfer_attachment_labels($lang);
...
if ($jobs !== []) {
    ...
    foreach ($jobs as $idx => $job) {
        ...
        try {
            ...
            $targetField = map_student_document_to_credit_attachment($documentType);
            if ($targetField === '' || !array_key_exists($targetField, $creditAttachmentLabels)) {
                $documents[] = [
                    'client_index' => $clientIndex,
                    'original_name' => $originalName,
                    'field' => '',
                ...
                $warnings[] = $originalName . ': this document type is not used on the credit transfer form.';
                continue;
            }
            ...
            $documents[] = [
                ...
                'original_name' => $originalName,
                'field' => $targetField,
                'field_label' => $creditAttachmentLabels[$targetField],
                'confidence' => $confidence,
                'summary' => $summary
            ];
...
$_SESSION['smart_autofill_batch_upload_token_credit'] = $uploadToken;
$_SESSION['smart_autofill_batch_upload_token_credit_expires'] = time() + 900;
json_exit([
    'status' => 'success',
    'message' => 'Documents analyzed successfully.',
    'fields' => map_merged_student_fields_to_credit_form($mergedFields),
    'documents' => $documents,
    'warnings' => $warnings,
