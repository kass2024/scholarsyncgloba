...
/**
 * Employment Opportunities E-Sign Contract — auto-create tables (idempotent).
 * Only touches eo_employment_contract* objects / upload folder.
 */
function eo_contract_ensure_schema(mysqli $conn): void
{
    static $ran = false;
    if ($ran) {
        return;
    ...
    $uploadDir = dirname(__DIR__) . '/uploads/eo_contracts';
    if (!is_dir($uploadDir)) {
        @mkdir($uploadDir, 0755, true);
    }

    $statements = [
        "CREATE TABLE IF NOT EXISTS `eo_employment_contracts` (
          ...
          `training_field` varchar(80) DEFAULT NULL,
          `program_fee` varchar(190) DEFAULT NULL,
        ...
        "CREATE TABLE IF NOT EXISTS `eo_employment_signatures` (
          `id` int(11) unsigned NOT NULL AUTO_INCREMENT,
